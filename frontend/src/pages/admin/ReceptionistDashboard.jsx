...
import { ArrowDown, ArrowUp, CalendarDays, Columns3, Pencil, Plus, Save, Trash2, X } from 'lucide-react';
...
const CellInput = ({ column, value, onChange }) => {
  ...
  if (column.type === 'file') return <div className="px-3 py-2"><input type="file" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => onChange(String(reader.result)); reader.readAsDataURL(file); }} className="max-w-[180px] text-xs" />{value?.startsWith('data:') && <a href={value} download="management-attachment" className="mt-1 block text-xs text-sage">View attachment</a>}</div>;
...
const ReceptionistDashboard = () => {
  ...
  const loadLayout = useCallback(async () => {
    const { data } = await api.get('/sales-sheet/layout', { params: { sheet: 'management' } });
  ...
  const descriptors = useMemo(() => [
    ...
    { key: 'lastUpdatedAt', label: 'Last Updated Date', fixed: true },
  ...
  const orderedDescriptors = useMemo(() => {
    ...
    return [...saved, ...descriptors.filter((descriptor) => !savedKeys.has(descriptor.key))];
  ...
  const saveLayout = async () => {
    await api.put('/sales-sheet/layout', { sheet: 'management', columns: orderedDescriptors.map((descriptor) => ({ key: descriptor.key })) });
    setArrangeOpen(false);
  };
  const span = orderedDescriptors.length;
    ...
    <div className="mx-auto max-w-[1800px]">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3 border-b border-cardline pb-5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button disabled={Boolean(draft)} onClick={() => { setDraft(emptyValues(columns)); setSalesDraft(emptyValues(salesColumns)); setEditingId(null); }}><Plus size={16} /> Add appointment</Button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto border border-cardline bg-offwhite-100">
        <table className="w-full min-w-max border-collapse text-left">
          <thead><tr className="bg-tan-100 text-xs uppercase text-charcoal/60">{orderedDescriptors.map((descriptor) => <th key={descriptor.key} className="min-w-[145px] border-r border-cardline px-3 py-3">{descriptor.label}{descriptor.column?.required && ' *'}</th>)}</tr></thead>
          <tbody>
      ...
      <Modal open={arrangeOpen} onClose={() => setArrangeOpen(false)} title="Arrange Appointment Management columns" className="max-w-3xl">
        <div className="divide-y divide-cardline">{orderedDescriptors.map((descriptor, index) => <div key={descriptor.key} className="flex items-center gap-3 py-3"><span className="w-6 text-xs text-charcoal/40">{index + 1}</span><span className="flex-1 font-semibold">{descriptor.label}</span><button disabled={!index} onClick={() => moveDescriptor(index, -1)} className="p-1 text-sage disabled:opacity-25" title="Move up"><ArrowUp size={16} /></button><button disabled={index === orderedDescriptors.length - 1} onClick={() => moveDescriptor(index, 1)} className="p-1 text-sage disabled:opacity-25" title="Move down"><ArrowDown size={16} /></button></div>)}</div>
